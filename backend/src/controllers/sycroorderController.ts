...
export async function getOrders(req: Request, res: Response): Promise<void> {
  try {
    ...
    if (pdNumbers.size > 0) {
      const foraDoGerenciador = orders.filter(
        (o) => o.status !== 'FINISHED' && !pdNumbers.has(String(o.order_number ?? '').trim())
      );
      for (const o of foraDoGerenciador) {
        ...
        await prisma.sycroOrderHistory.create({
          data: {
            ...
            user_id: null,
            user_name: 'Sistema',
            action_type: 'AUTO_ATENDIDO',
            previous_date: o.current_promised_date,
            new_date: o.current_promised_date,
            observation: 'Pedido não está mais na listagem do Gerenciador de Pedidos.',
          },
        });
      }
      if (foraDoGerenciador.length > 0) {
    ...
    const list = ordersParaListar.map((o) => {
      ...
      const canRespond =
        !hasResponsavel ||
        (currentUserId != null && (o.created_by === currentUserId || (login && login.toLowerCase() === 'josenildo')));
      return {
        id: o.id,
...
export async function updateOrder(req: Request, res: Response): Promise<void> {
  ...
  const { status, new_date, observation, comentario, observacao, is_urgent, motivo, id_pedidos } = req.body as {
    status?: OrderStatus;
    new_date?: string;
  ...
  try {
    ...
    const canRespond =
      !hasResponsavel ||
      (user_id != null && (order.created_by === user_id || (usuario?.login && usuario.login.toLowerCase() === 'josenildo')));
    if (!canRespond) {
      res.status(403).json({ error: 'Apenas o criador do card ou o responsável (josenildo) podem responder este pedido.' });
      return;
    }

    const nextDate = new_date !== undefined && new_date !== null ? String(new_date).trim() : order.current_promised_date;
    if (new_date !== undefined && new_date !== null && String(new_date).trim() !== '') {
    ...
    if (order.status === 'PENDING') {
      ...
      nextStatus = (status && ['PENDING', 'FINISHED', 'ESCALATED'].includes(status) ? status : order.status) as OrderStatus;
    ...
    const statusFinaisPermitidos = new Set(['Atendido totalmente', 'Atendido com corte', 'Cancelado']);
    if (nextStatus === 'FINISHED') {
      try {
        const { data: gerenciadorList } = await listarPedidos({});
        const orderNumber = String(order.order_number ?? '').trim();
        const itensPedido = gerenciadorList.filter(
          (row: Record<string, unknown>) => String(row['PD'] ?? '').trim() === orderNumber
        );
        const stautsField = (row: Record<string, unknown>) => String(row['Stauts'] ?? row['Status'] ?? '').trim();
        const itensNaoPermitidos = itensPedido.filter((row) => !statusFinaisPermitidos.has(stautsField(row)));
        if (itensNaoPermitidos.length > 0) {
          res.status(400).json({
            error:
              'O pedido só pode ir para Faturado/Entregue quando todos os itens estiverem com status no ERP: Atendido totalmente, Atendido com corte ou Cancelado. Ainda há itens com outro status.',
          });
          return;
        }
      } catch (errCheck) {
        console.error('sycroorder updateOrder: verificar status itens', errCheck);
        res.status(503).json({ error: 'Não foi possível verificar o status dos itens no ERP.' });
        return;
      }
    }
